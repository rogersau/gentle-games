...
- Dedicated formatter config lives in `.oxfmtrc.json` at the repository root.
- `oxfmt` is configured to prefer single quotes in JavaScript/TypeScript and JSX (`singleQuote: true`, `jsxSingleQuote: true`).
...
- Quote style is standardized to single quotes by the formatter. If you touch legacy files with mixed quotes, reformat the file rather than introducing new double-quoted literals.
